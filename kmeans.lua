...
function kmeans(x, k, niter, batchsize, callback, verbose)
   ...
   local totalcounts = x.new(k):zero()
	local assign = torch.LongTensor(nsamples):zero()
   ...
   for i = 1,niter do
      ...
      for i = 1,nsamples,batchsize do
		...
		print( ( 'Kmeans) Iter. %d, loss: %.2f' ):format( i, loss ) )
   ...
   -- done
   return centroids:reshape(k_size),assign,totalcounts
end
